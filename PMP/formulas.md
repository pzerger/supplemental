...
This document Contains a list of formulas and formula-related  terms you should know for the PMP exam, along with when the formula should be used, along with the formula itself.
...
PERT Beta is a weighted average calculation used in project management to estimate the expected duration of an activity.
...
PERT Standard Deviation measures the variability or uncertainty in the PERT estimates.
...
The Triangular Distribution is a simpler alternative to the PERT Beta, using only three estimates without weighting.
...
PERT (Program Evaluation and Review Technique) formulas are used for estimating project durations and analyzing schedule network diagrams.
...
TCPI measures the cost performance that must be achieved to complete the remaining work within a specified budget.
...
The number of potential communication channels between project stakeholders.
...
CPI measures the cost efficiency of the project, comparing earned value to actual costs.
...
The total cost incurred for work performed on the project to date.
...
The value of work actually completed, expressed in terms of the budget assigned to that work.
...
The expected cost to complete all remaining project work.
...
The projected difference between the budget at completion and the estimate at completion.
...
A technique used to calculate the average outcome of scenarios that may or may not happen.
...
SPI measures the schedule efficiency of the project, comparing earned value to planned value.
...
A contract type where the seller is reimbursed for costs plus an award fee based on performance.
...
A contract type where the seller is reimbursed for costs plus a fixed fee payment.
...
A contract type where the seller is reimbursed for costs plus an incentive fee based on meeting defined performance targets.
...
A numeric assessment of risk used in Failure Mode and Effects Analysis (FMEA).
...
A contract type where the seller is reimbursed for costs plus a percentage of costs as profit.
...
A ratio used to summarize the overall value for money of a project or proposal.
...
In a Fixed Price Incentive Fee (FPIF) contract, the point at which the seller assumes all additional costs.
...
In incentive contracts, the estimated cost plus target profit that represents the expected final price.
...
## PMP Formula Study Reference

Here's a table with each term and its associated formula to help you prepare.
